...
const groups = [
  {
    label: 'Overview',
    items: [{ label: 'Dashboard', path: '/trainee/dashboard', icon: <DashboardOutlined /> }],
  },
  {
    label: 'Learning',
    items: [
...
const isActive = (pathname: string, path: string) =>
  pathname === path ||
  (path === '/trainee/courses' && pathname.startsWith('/trainee/courses/')) ||
  (path === '/trainee/assessments' && pathname.startsWith('/trainee/assessments/'));
  ...
  const drawerContent = (isMobile = false) => (
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <Box sx={{ px: isMobile || !collapsed ? 2.5 : 1, py: 2, minHeight: 72, display: 'flex', alignItems: 'center', justifyContent: isMobile || !collapsed ? 'flex-start' : 'center' }}>
        {isMobile || !collapsed ? (
          <Box>
            <Typography variant="h6" color="primary" noWrap sx={{ fontWeight: 800 }}>CAPACITY CONNECT</Typography>
            <Typography variant="caption" color="text.secondary">Trainee Portal</Typography>
          </Box>
        ) : <Typography variant="h6" color="primary" sx={{ fontWeight: 800 }}>CC</Typography>}
      </Box>
      ...
      <List sx={{ px: 1, py: 1, flex: 1, overflowY: 'auto' }}>
        {groups.map((group) => (
          <Box key={group.label} sx={{ mb: 1.5 }}>
            {(!collapsed || isMobile) && (
              <Typography sx={{ px: 2, py: .75, color: 'text.secondary', fontSize: '.68rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '.08em' }}>
                {group.label}
              </Typography>
            )}
                ...
                <ListItemButton selected={isActive(location.pathname, item.path)} onClick={() => handleNavigation(item.path)} sx={{ minHeight: 46, px: collapsed && !isMobile ? 1.5 : 2, justifyContent: collapsed && !isMobile ? 'center' : 'flex-start', borderRadius: 2, mb: .35 }}>
                  ...
                  {(!collapsed || isMobile) && (
                    <Box sx={{ display: 'flex', alignItems: 'center', width: '100%', gap: 1 }}>
                      ...
                      {item.isNew && <Box component="span" sx={{ bgcolor: '#ef4444', color: '#fff', fontSize: 9, fontWeight: 800, px: .7, py: .25, borderRadius: 1 }}>NEW</Box>}
                    </Box>
                  )}
                </ListItemButton>
              </Tooltip>
      ...
      <Box sx={{ p: 1, borderTop: '1px solid', borderColor: 'divider' }}>
        <Tooltip title={collapsed && !isMobile ? 'Settings' : ''} placement="right">
          <ListItemButton selected={location.pathname === '/trainee/settings'} onClick={() => handleNavigation('/trainee/settings')} sx={{ minHeight: 48, px: collapsed && !isMobile ? 1.5 : 2, justifyContent: collapsed && !isMobile ? 'center' : 'flex-start', borderRadius: 2 }}>
    ...
    <Box sx={{ display: 'flex', minHeight: '100vh', bgcolor: 'background.default' }}>
      <AppBar position="fixed" color="inherit" elevation={0} sx={{ zIndex: (theme) => theme.zIndex.drawer + 1, width: { xs: '100%', md: `calc(100% - ${collapsed ? collapsedWidth : drawerWidth}px)` }, ml: { xs: 0, md: `${collapsed ? collapsedWidth : drawerWidth}px` }, transition: 'width .2s, margin-left .2s', borderBottom: '1px solid', borderColor: 'divider', bgcolor: 'background.paper' }}>
        <Toolbar>
          ...
          <Typography variant="h6" color="primary" noWrap sx={{ flexGrow: 1, fontWeight: 700 }}>Trainee Portal</Typography>
          <IconButton onClick={() => navigate('/trainee/notifications')} aria-label="Notifications"><NotificationsOutlined /></IconButton>
          <IconButton onClick={() => navigate('/trainee/settings')} aria-label="Settings"><SettingsOutlined /></IconButton>
        </Toolbar>
      </AppBar>
      ...
      <Box component="main" sx={{ flexGrow: 1, minWidth: 0 }}>
        <Toolbar />
        <Box sx={{ p: { xs: 2, sm: 3, md: 4 } }}><Outlet /></Box>
      </Box>
    </Box>
  );
};
